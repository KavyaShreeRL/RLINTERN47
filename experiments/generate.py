...
def generate_greedy(model, start_text, length, stoi, itos, device):
    model.eval()
    idx = torch.tensor([stoi[c] for c in start_text if c in stoi], dtype=torch.long, device=device).unsqueeze(0)
    if idx.size(1) == 0:
        idx = torch.tensor([[0]], dtype=torch.long, device=device)  # fallback to first char
...
def main():
    ...
    while True:
        ...
        if start_seq.lower() == "exit":
            break
        generated_text = generate_greedy(model, start_seq, args.max_new_tokens, stoi, itos, device)
        print("\nGenerated Text:\n")
